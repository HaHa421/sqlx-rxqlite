...
#[test]
fn many_connections_with_options() {
    let rt = Runtime::new().unwrap();
    let _ = rt.block_on(async {
        //const QUERY: &str ="SELECT name,birth_date from _test_user_ where name = ?";
        let /*mut*/ tm = TestManager::new("many_connections_with_options",3,None);
        tm.wait_for_cluster_established(1, 60).await.unwrap();
        const CONNECTIONS_COUNT:usize=100;
        let mut futs = vec![];
        let mut rng = rand::thread_rng();
        for i in 0..CONNECTIONS_COUNT {
          ...
          futs.push(async move{
            ...
            connect_options = connect_options.node_id(node_id as _);
            let mut split= http_addr.split(":");
